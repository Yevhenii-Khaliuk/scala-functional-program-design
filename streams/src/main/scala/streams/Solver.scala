...
trait Solver extends GameDef:
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList((startBlock, Nil)), Set.empty)
  ...
  lazy val solution: List[Move] =
    val possibleSolution = pathsToGoal.take(1)
    if possibleSolution.isEmpty then Nil else possibleSolution.head._2.reverse
